...
#define OUTPUT_MODE_SOCKET  3 //new
...
static void NnparseArgs(param_info_t *param, int argc, char** argv)
{
	...
	while(-1 != (opt = getopt(argc, argv, "i:o:w:W:h:H:n:N:p:P:X:Y:g:u:a::t?")))
	{
		switch (opt) {
			// Common Parameter
			case 'i':
			...
			case 'o':
				...
				else if(strcmp(optarg, "file") == 0)
				{
   				...
   				{
       				param->outputMode = OUTPUT_MODE_SOCKET;
       				param->outputFormat = IMAGE_FMT_RGB24; 
    			} else
				{
					printf("invalid parameter %s", optarg);
...
static void NnOutputModeInit(app_context_t *pContext, DisplayHandle dispHandle, MessageHandle msgHandle)
{
	...
	switch (output_mode)
	{
		...
		case OUTPUT_MODE_SOCKET:
           
            if ( (pContext->custom_sockfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0 ) {
                perror("[ERROR] Socket creation failed");
                exit(EXIT_FAILURE);
            }

            
            memset(&pContext->custom_servaddr, 0, sizeof(pContext->custom_servaddr));
            pContext->custom_servaddr.sin_family = AF_INET;
            pContext->custom_servaddr.sin_port = htons(8080); 
            
            
            if (pContext->pRtpmIpAdress != NULL) {
                pContext->custom_servaddr.sin_addr.s_addr = inet_addr(pContext->pRtpmIpAdress);
            } else {
                pContext->custom_servaddr.sin_addr.s_addr = inet_addr("192.168.0.8"); 
            }
            
            printf("[INFO] Custom UDP Socket Initialized. Target: %s:8080\n", pContext->pRtpmIpAdress);
            break;
	}
...
static void NnOutputModeDeinit(app_context_t *pContext, DisplayHandle dispHandle, MessageHandle msgHandle)
{
	...
	if (pContext->outputDataType == OUTPUT_MODE_SOCKET) 
    {
        close(pContext->custom_sockfd);
        printf("[INFO] Custom UDP Socket Closed.\n");
    }
	else
	{
		// TBD
...
static int32_t NnOutputResultData(app_context_t *pContext, MessageHandle msgHandle)
{
	...
	else if (output_mode == OUTPUT_MODE_SOCKET) 
    {
        network_context_t *pNeuralNetwork = &pContext->inferenceContext.neuralNetwork[NETWORK_INDEX_0];
        
        if (pNeuralNetwork->type == TELECHIPS_NPU_POST_CUSTOM) 
        {
            char payload[1024];
            int hand_count = pNeuralNetwork->hand_data.num_det;

            
            if (hand_count > 0) {
                hpd_detection_t *det = &pNeuralNetwork->hand_data.dets[0]; // 첫 번째 손
                sprintf(payload, "{\"hand_cnt\": %d, \"score\": %.2f, \"box\": [%.1f, %.1f, %.1f, %.1f]}\n", 
                        hand_count, det->score, det->x1, det->y1, det->x2, det->y2);
            } else {
                sprintf(payload, "{\"hand_cnt\": 0}\n");
            }

            
            sizeRet = sendto(pContext->custom_sockfd, (const char *)payload, strlen(payload),
                             MSG_CONFIRM, (const struct sockaddr *) &pContext->custom_servaddr, 
                             sizeof(pContext->custom_servaddr));
